...
const blurredImg = document.querySelectorAll('.js-img-blur');
...
const workHeroImages = document.querySelectorAll('.js-work-hero');
const workHeroHover = document.querySelectorAll('.js-work-hover');
const projectAmount = document.querySelector('.js-project-total');
const projectTotal = document.querySelectorAll('.o-project');
const labAmount = document.querySelector('.js-experiments-total');
const labTotal = document.querySelectorAll('.o-lab');
const projectNumberOrder = document.querySelectorAll('.js-project-number');
const labNumberOrder = document.querySelectorAll('.js-lab-number');
// parallax and cursor
...
// const contactCopyLinks = document.querySelector('.js-links');
...
//////
//// hero hover image change
workHeroHover.forEach((entry, index) => {
  entry.addEventListener('mouseover', function() {
    if (index === 0) {
      workHeroImages[0].classList.remove('js-hide')
      workHeroImages[1].classList.add('js-hide')
    } if (index <= 1) {
      workHeroImages[index].classList.remove('js-hide')
    } else { workHeroImages[index].classList.add('js-hide')
  }
  });
});

// project and experiment total tracking
function worksRunningTotals(target, sum) {
  let runningTotal = '0' + sum.length;
  target.innerText = runningTotal
};

worksRunningTotals(projectAmount, projectTotal);
worksRunningTotals(labAmount, labTotal);

//////
//// project number tracking per project
projectNumberTracking = function(entry, index) {
  number = index + 1;
  entry.innerText = '/0' + number;
};
projectNumberOrder.forEach((entry, index) => {
  projectNumberTracking(entry, index);
});
labNumberOrder.forEach((entry, index) => {
  projectNumberTracking(entry, index);
});